...
export default function NewGeoaltBlogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/blogs', {
        ...
        body: JSON.stringify({ title, slug, description, image, content, platform: 'geoalt', metaTitle, metaDescription, tags }),
      });
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-8">
        <button onClick={() => router.push('/geoalt')} className="flex items-center gap-2 text-gray-500 hover:text-gray-700 transition-colors mb-4 text-sm font-medium">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          Back to GeoAlt
        </button>
        <h1 className="text-2xl font-bold text-gray-900">Create New Blog</h1>
        <p className="text-gray-500 mt-1">Add a new blog post to GeoAlt</p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button type="button" onClick={() => router.push('/geoalt')} className="w-full py-3.5 px-6 border border-gray-300 text-gray-700 font-semibold rounded-xl hover:bg-gray-50 transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      </form>
